...
chat = client.chats.create(model="gemini-3.1-flash-lite") # Initialize the model
...
def get_completion_from_messages(messages, temperature=0.7, max_tokens=500):
    system_message =""
    user_messages=""
    for message in messages:
        if message["role"] == "system":
            ...
            user_messages = message["content"]  # Store the user message
        else:
            raise ValueError(f"Message missing 'content' field: {message}")  # Debugging
    response= chat.send_message(user_messages,
                                config= types.GenerateContentConfig(systemInstruction=system_message,
